<?php
    ...
    if ($hurrengoa === "form") {
        ...
        $sql = "SELECT A.id as id, B.izena as izena, C.izena as ariketa_mota
                    FROM ariketak A
                    INNER JOIN ariketak_hizkuntzak AS B
                    ON A.id = B.fk_elem
                    INNER JOIN ariketa_motak_hizkuntzak AS C
                    ON A.fk_ariketa_mota = C.fk_elem
                    WHERE B.fk_hizkuntza = " . $hizkuntza["id"] . " AND C.fk_hizkuntza = " . $hizkuntza["id"] . "
                    AND A.egoera = 1
