...
<div class="navbar">
	<div class="navbar-inner">
		<div class="brand"><a href="<?php echo URL_BASE_ADMIN; ?>ikasgaiak">Ikasgaiak</a> > <?php if ($edit_id) { echo $ikasgaia->hizkuntzak[$hizkuntza["id"]]->izenburua; } else { echo "Gehitu berria"; } ?></div>
	...
	<form id="f1" name="f1" method="post" action="<?php echo $url_base . "form" . $url_param; ?>" class="form-horizontal" enctype="multipart/form-data" onsubmit="javascript: return verif();">
		...
		<fieldset>
			...
			<div class="control-group">
			    ...
			    <input class="input-large datetimepicker" name="hasiera_data" type="text" id="hasiera_data" value="<?php echo $ikasgaia->hasiera_data?>" />
			</div>
			<div class="control-group">
			    <label for="hasiera_data">Bukaera data:</label>
			    <input class="input-large datetimepicker" name="bukaera_data" type="text" id="bukaera_data" value="<?php echo $ikasgaia->bukaera_data?>" />
			</div>
		...
		<fieldset>
			...
			<div class="control-group">
			    ...
			    <select id="konbo_ariketak"  name="ariketak[]" multiple="multiple">
						...
						<option <?php if(in_array($ariketa['id'], $ariketa_idak)){?>selected="selected"<?php }?> value="<?=$ariketa['id']?>" ><?=$ariketa['izena']?></option>
						<?php }?>
				</select>
