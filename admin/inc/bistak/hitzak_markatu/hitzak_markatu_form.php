...
<div class="navbar">
	<div class="navbar-inner">
		<div class="brand"><a href="<?php echo URL_BASE_ADMIN; ?>hitzak-markatu">Hitzak markatu</a> > <?php if ($edit_id) { echo $hitzak_markatu->hizkuntzak[$h_id]->izena; } else { echo "Gehitu berria"; } ?></div>
	...
	<form id="f1" name="f1" method="post" action="<?php echo $url_base . "form" . $url_param; ?>" class="form-horizontal" enctype="multipart/form-data" onsubmit="javascript: return verif();">
        ...
        <fieldset>
            ...
            <div class="control-group">
			    ...
			    <select id="konbo-ikus-entzunezkoak" class="input-xxlarge" name="ikus-entzunezkoa">
                    <?php if (!$hitzak_markatu->ikus_entzunezkoa->id) { ?>
                    <option disabled selected value="">HAUTATU IKUS-ENTZUNEZKOA</option>
                    <?php } ?>
					<?php foreach($ikus_entzunezkoak as $ikus_entzunezkoa){?>
					<option <?php if($ikus_entzunezkoa['id'] == $hitzak_markatu->ikus_entzunezkoa->id){?>selected="selected"<?php }?> value="<?=$ikus_entzunezkoa['id']?>" ><?php echo $ikus_entzunezkoa['izenburua'] . " [" . $ikus_entzunezkoa["mota"] . "]"; ?></option>
					<?php }?>
				</select>
			</div>
            ...
            <div>
                <div>
                    <video id="bideoa-aurrebista-erreproduktorea" controls<?php if ($hitzak_markatu->ikus_entzunezkoa->mota != "bideoa") { echo " style='display:none'";} ?>>
                        <source id="bideoa-aurrebista-erreproduktorea-mp4" src="<?php echo URL_BASE . $hitzak_markatu->ikus_entzunezkoa->bideo_path . $hitzak_markatu->ikus_entzunezkoa->bideo_mp4; ?>" type="video/mp4"></source>
                        <source id="bideoa-aurrebista-erreproduktorea-webm"  src="<?php echo URL_BASE . $hitzak_markatu->ikus_entzunezkoa->bideo_path . $hitzak_markatu->ikus_entzunezkoa->bideo_webm; ?>" type="video/webm"></source>
                    </video>
                </div>
                
                <div>
                    <audio id="audioa-aurrebista-erreproduktorea" controls<?php if ($hitzak_markatu->ikus_entzunezkoa->mota != "audioa") { echo " style='display:none'";} ?>>
                        <source id="audioa-aurrebista-erreproduktorea-mp3" src="<?php echo URL_BASE . $hitzak_markatu->ikus_entzunezkoa->audio_path . $hitzak_markatu->ikus_entzunezkoa->audio_mp3; ?>" type="audio/mpeg"></source>
                        <source id="audioa-aurrebista-erreproduktorea-ogg"  src="<?php echo URL_BASE . $hitzak_markatu->ikus_entzunezkoa->audio_path . $hitzak_markatu->ikus_entzunezkoa->audio_ogg; ?>" type="audio/ogg"></source>
                    </audio>
                </div>
                
                <div id="ikus-entzunezkorik-ez"<?php if ($hitzak_markatu->ikus_entzunezkoa) { echo " style='display:none;'"; } ?>>Galdera-erantzun ariketa honek ez dauka ikus-entzunezkorik oraindik.</div>
            </div>
            ...
            <div class="control-group">
			    <select id="konbo-dokumentuak" class="input-xxlarge" name="dokumentuak[]" multiple="multiple">
					<?php foreach($dokumentuak as $dokumentua){?>
                        <option 
                        <?php if ($hitzak_markatu->dokumentuak) {
                            foreach($hitzak_markatu->dokumentuak as $ariketaren_dokumentua) { ?>
		...
		<fieldset>
			...
			<div class="control-group">
				...
				<input class="input-xxlarge" type="text" id="izena_<?php echo $h_id; ?>" name="izena_<?php echo $h_id; ?>" value="<?php echo testu_formatua_input ($hitzak_markatu->hizkuntzak[$h_id]->izena); ?>" />
			</div>
            
            <div class="control-group">
				<label for="azalpena_<?php echo $h_id; ?>">Azalpena:</label>
				<textarea class="input-xxlarge" id="azalpena_<?php echo $h_id; ?>" name="azalpena_<?php echo $h_id; ?>"><?php echo testu_formatua_input($hitzak_markatu->hizkuntzak[$h_id]->azalpena); ?></textarea>
			</div>
