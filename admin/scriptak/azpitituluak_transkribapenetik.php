...
$transkribapena = $_POST['transkribapena'];
...
if(!is_file($fitxategia)){
    ...
    $txt_berria = TRANSKRIBAPENETIK_DIR . $row["id"] . "-" . $pathinfo["basename"] . ".txt";
    if (!file_put_contents($txt_berria, $transkribapena)) {
        $erantzuna = array('erantzuna' => 'Transkribapena ezin izan da kopiatu');
        echo json_encode($erantzuna);
        exit;
    }
